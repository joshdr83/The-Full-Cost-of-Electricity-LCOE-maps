E_make_min_cost_map_exclusions_with_ext <- function(){
...
## LCOE 3 ###
	...
	coal_bit3 <- coal_bit[c('FIPS', 'LCOE3')]
	coal_bit3 <- merge(x = coal_bit3, y = coal_ex, by = 'FIPS', all.x = T)
	coal_bit3$LCOE3b <- coal_bit3$LCOE3 * coal_bit3$can_build
	coal_bit3 <- coal_bit3[c('FIPS', 'LCOE3b')]	
		
	coal_sub3 <- coal_sub[c('FIPS', 'LCOE3')]
	coal_sub3 <- merge(x = coal_sub3, y = coal_ex, by = 'FIPS', all.x = T)
	coal_sub3$LCOE3b <- coal_sub3$LCOE3 * coal_sub3$can_build
	coal_sub3 <- coal_sub3[c('FIPS', 'LCOE3b')]
	
	ngcc3 <- ngcc[c('FIPS', 'LCOE3')]
	ngcc3 <- merge(x = ngcc3, y = ngcc_ex, by = 'FIPS', all.x = T)
	ngcc3$LCOE3b <- ngcc3$LCOE3 * ngcc3$can_build
	ngcc3 <- ngcc3[c('FIPS', 'LCOE3b')]
	
	
	ngct3 <- ngct[c('FIPS', 'LCOE3')]
	ngct3 <- merge(x = ngct3, y = ngct_ex, by = 'FIPS', all.x = T)
	ngct3$LCOE3b <- ngct3$LCOE3 * ngct3$can_build
	ngct3 <- ngct3[c('FIPS', 'LCOE3b')]
	
	nuke3 <- nuke[c('FIPS', 'LCOE3')]
	nuke3 <- merge(x = nuke3, y = nuke_ex, by = 'FIPS', all.x = T)
	nuke3$LCOE3b <- nuke3$LCOE3 * nuke3$can_build
	nuke3 <- nuke3[c('FIPS', 'LCOE3b')]
	
	pv_res3 <- pv_res[c('FIPS', 'LCOE3')]
	
	pv_util3 <- pv_util[c('FIPS', 'LCOE3')]
	pv_util3 <- merge(x = pv_util3, y = pv_wind_ex, by = 'FIPS', all.x = T)
	pv_util3$LCOE3b <- pv_util3$LCOE3 * pv_util3$can_build
	pv_util3 <- pv_util3[c('FIPS', 'LCOE3b')]
	
	wind3 <- wind[c('FIPS', 'LCOE3')]
	wind3 <- merge(x = wind3, y = pv_wind_ex, by = 'FIPS', all.x = T)
	wind3$LCOE3b <- wind3$LCOE3 * wind3$can_build
	wind3 <- wind3[c('FIPS', 'LCOE3b')]
	
	csp3 <- csp[c('FIPS', 'LCOE3')]
	csp3 <- merge(x = csp3, y = csp_ex, by = 'FIPS', all.x = T)
	csp3$LCOE3b <- csp3$LCOE3 * csp3$can_build
	csp3 <- csp3[c('FIPS', 'LCOE3b')]
	
	coal_bit3_ccs <- coal_bit_ccs[c('FIPS', 'LCOE3')]
	coal_bit3_ccs <- merge(x = coal_bit3_ccs, y = coal_ccs_ex, by = 'FIPS', all.x = T)
	coal_bit3_ccs$LCOE3b <- coal_bit3_ccs$LCOE3 * coal_bit3_ccs$can_build
	coal_bit3_ccs <- coal_bit3_ccs[c('FIPS', 'LCOE3b')]
	
	coal_sub3_ccs <- coal_sub_ccs[c('FIPS', 'LCOE3')]
	coal_sub3_ccs <- merge(x = coal_sub3_ccs, y = coal_ccs_ex, by = 'FIPS', all.x = T)
	coal_sub3_ccs$LCOE3b <- coal_sub3_ccs$LCOE3 * coal_sub3_ccs$can_build
	coal_sub3_ccs <- coal_sub3_ccs[c('FIPS', 'LCOE3b')]
	
	ngcc3_ccs <- ngcc_ccs[c('FIPS', 'LCOE3')]
	ngcc3_ccs <- merge(x = ngcc3_ccs, y = ngcc_ccs_ex, by = 'FIPS', all.x = T)
	ngcc3_ccs$LCOE3b <- ngcc3_ccs$LCOE3 * ngcc3_ccs$can_build
	ngcc3_ccs <- ngcc3_ccs[c('FIPS', 'LCOE3b')]
	...
	names(merge3) <- c('FIPS', 'LCOE3', 'tech')
	...
	tech3 <- final3
	names(tech3) <- c('tech' , 'FIPS' ,'LCOE3' , 'color')
	tech_lcoe <- tech3
  ...
  pdf(file = paste('Min_cost_map_with_ext_exclusions.pdf', sep = ''), height = 7, width = 9)
  ...
  map("state", col = "black", fill = FALSE, add = TRUE, lty = 1, lwd = 0.2,
    projection="polyconic")  
  mtext(text = 'Scenario 3: with availability zones and externalities', cex = 2, line = -1.25) #, col = 'white')
